...
const Register = () => {
  ...
  const [role, setRole] = useState<"recruiter" | "seeker">("seeker");

  const roles = [
    { value: "seeker" as const, label: "Je cherche un emploi", icon: User, desc: "Parcourez les offres et postulez" },
    { value: "recruiter" as const, label: "Je recrute", icon: Briefcase, desc: "Publiez des offres et trouvez des talents" },
  ];
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex w-full items-center justify-center px-6 lg:w-1/2">
        <div className="w-full max-w-md">
          <h1 className="mb-2 font-heading text-2xl font-bold text-foreground">Créer un compte</h1>
          <p className="mb-6 text-sm text-muted-foreground">Choisissez votre profil et commencez</p>
          ...
          <div className="mb-6 grid grid-cols-2 gap-3">
            {roles.map((r) => (
              ...
              >
                <r.icon className={`mb-2 h-5 w-5 ${role === r.value ? "text-primary" : "text-muted-foreground"}`} />
          ...
          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            ...
            <Button variant="hero" className="w-full" size="lg">Créer mon compte</Button>
          </form>
